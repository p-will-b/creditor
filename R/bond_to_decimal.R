...
bond_to_decimal <- function(bond_px) {
  ...
  frac <- .Internal(unlist(x = lapply(X = split_px, FUN = `[`, 2L), recursive = FALSE, use.names = FALSE))
  ...
  idx2 <- grepl(pattern = "\\+$", x = frac, perl = TRUE)
  frac[idx2] <- gsub(pattern = "\\+$", replacement = "/32 1/64", x = frac[idx2], perl = TRUE)
  idx3 <- which(nchar(x = frac) == 3L & !grepl(pattern = "[^0-9]", x = frac, perl = TRUE))
  frac[idx3] <- paste0(.Internal(substr(x = frac[idx3], start = 1L, stop = 2L)), "/32 ", .Internal(substr(x = frac[idx3], start = 3L, stop = 3L)), "/8*1/32")
  idx4 <- which(x = !grepl(pattern = "[^0-9]", x = frac, perl = TRUE))
  frac[idx4] <- paste0(frac[idx4], "/32")
  idx5 <- which(x = grepl(pattern = "\\d\\.\\d", x = frac, perl = TRUE))
